...
func (s *Service) ScheduleImport(ctx context.Context, input ScheduleImportInput) error {
	start := time.Now()
	...
	err = s.transactor.WithinTransaction(ctx, func(repo postgres.Repository) error {
		return repo.Schedule().UpSetWeekSchedule(ctx, &input.WeekSchedule)
	})
...
func (s *Service) GetScheduleByCourse(ctx context.Context, course int, filters domain.ScheduleFilters) (*domain.ScheduleCourse, error) {
	if course <= 0 {
